...
class RunTitle {
    ...
    void Death(String title, String subtitle, World world, int delay){
        ...
        Bukkit.getServer().getScheduler().runTaskLaterAsynchronously(KeepMyLife.plugin, new Runnable() {
            @Override
            public void run() {
                Title.create(RunTitle.title_a, Title.Type.TITLE, 40, 80, 40).sendWorld(RunTitle.world_a);
                Title.create(RunTitle.subtitle_a, Title.Type.SUBTITLE, 40, 80, 40).sendWorld(RunTitle.world_a);
            }
        }, delay);
    }
}
